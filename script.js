		...
		comments: [
			{
				name: "Sandra E.",
				comment:
					"Die zehn Thesen zum guten Design sind zeitlos. Pflichtlektüre.",
			},
			{
				name: "Designstudent",
				comment:
					"Bildband und Nachschlagewerk in einem. Der Druck ist hervorragend.",
			},
			{
				name: "Lindadesign",
				comment:
					"Schwer, teuer, jeden Cent wert. Liegt bei mir dauerhaft auf dem Nachttisch.",
			},
		],
	},
	{
		...
		comments: [
			{
				name: "Design.Guru",
				comment:
					"Erstaunlich unbeschönigt für eine autorisierte Biografie.",
			},
			{
				name: "Nick T.",
				comment:
					"Die Kapitel zu NeXT fand ich spannender als die iPhone-Jahre.",
			},
		],
	},
	{
	...
	return /*html*/ `
        <div class="book-card">
            <div class="container-product">          
                ...
                <h3>${books[i].title}</h3>
				${getProductDetailsTemplate(i)}
            </div>
            ${getCommentFormTemplate(i)}
        </div>       
    `;
}

function getLikePriceTemplate(i) {
	return /*html*/ `
...
function getProductDetailsTemplate(i) {
	return /*html*/ `
...
function getCommentFormTemplate(i) {
	return /*html*/`
		<div class="form-comment">
            <input class="input-field" type="text" id="comment-input-${i}" placeholder="Meinung schreiben...">
            <button class="send-btn" onclick="addComment(${i})"><img src="assets/icons/send.svg" alt="Meinung senden"></button>
        </div>  
	`;
}
...
function getCommentsTemplate(i) {
	if (books[i].comments.length === 0) {
		return `<p class="no-comments">Sei der Erste, der eine Meinung teilt.</p>`;
	}
	let commentsHTML = "";
